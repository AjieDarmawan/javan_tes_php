...
{   
    ...
    function event()
    {
        ...
        $sql = $this->db->query("select e.*,j.kategori_nama,j.id_kategori from event as e 
        inner JOIN kategori as j on e.id_kategori=j.id_kategori
                where  e.mode='event' and kedinasan = 0")->result();
...

    {
        ...
        $sql = $this->db->query("select e.*,j.kategori_nama,j.id_kategori from event as e 
        inner JOIN kategori as j on e.id_kategori=j.id_kategori
                where  e.mode='latihan' group by e.judul,e.tgl_mulai,e.tgl_selesai order by e.id_event desc")->result();
...
// for sending data as json type
          ...
          //   $template_mail = 'webinar-zoom';
             ...
             //$template_mail = 'webinar-wa';
              $pulish_zoom = 0;
                ...
                $gambar = base_url("assets/file_upload/webinar/" . $k->img_mobile);
                $data[] = array(
    ...
    {
        $email = $this->input->post('email');
        $id_event2 = $this->input->post('id_webinar');
        ...
        if ($cek) {
            ...
            $data['datanya'] = array(
                ...
                "jurusan_diinginkan"=> "",
                "sumber_informasi"=> "1",
                "create_add"=> "2022-04-21 15:33:39",
                "kategori"=> "",
                "kampus_favorit"=> "",
                "rating"=> "",
...
e.*,j.kategori_nama,j.id_kategori from event as e 
        inner JOIN kategori as j on e.id_kategori=j.id_kategori
          where  e.mode='event' and e.kedinasan= 2 group by e.desc ")->result();
                ...
                if ($s->tgl_selesai >= $hariini) {
                    ...
                    $mystring = $judul_grup;
                        ...
                        if(strpos($mystring, $word) !== false){
                            $kampus_cari = "Universitas Indonesia";
                        } else{
                            $kampus_cari =  "Universitas Gadjah Mada";
                        }




                    $data_group = $this->db->query("select TO_BASE64(e.id_event) as id_event2, (select id_jenis from materi where id_event = e.id_event limit 1) as id_jenis2, 
                        (select publish from materi where id_event = e.id_event limit 1) as publish, 
                        (select sum(waktu) from materi where publish = 1 and id_event = e.id_event) AS waktu,
                        (select count(id)  from soalonline as s
                            inner join materi as m on s.materi_id = m.materi_id

                            where m.id_event = e.id_event )as jumlah_soal,
                        e.*,j.kategori_nama,j.id_kategori from event as e 
                    inner JOIN kategori as j on e.id_kategori=j.id_kategori
