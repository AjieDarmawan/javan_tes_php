...
class Hasil_latihan extends CI_Controller
{
    ...
    public function index()
    {
       // $sess = $this->session->userdata();
        ...
        // die;
        //$data["Event"] = $this->Hasil_latihan_M->getAll();
        $data["title"] = "List Hasil  Event";
        $this->template->load('template','hasil_latihan/hasil_latihan_v',$data);
    ...
    public function ajax_list()
    {
        ...
        $list = $this->Hasil_latihan_M->get_datatables();
        $data = array();
        $no = $this->input->post('start');
        //looping data mahasiswa
        foreach ($list as $data_materi) {

            $waktu_awal        =strtotime($data_materi->tgl_mengerjakan);
            $waktu_akhir    =strtotime($data_materi->create_add); // bisa juga waktu sekarang now()
            
            //menghitung selisih dengan hasil detik
            $diff    =$waktu_akhir - $waktu_awal;

            $edit = "<a data-toggle='tooltip' title='Edit'  href=".base_url('hasil/Hasil_latihan/list/'.base64_encode($data_materi->id_event))."><button class='btn btn-success btn-xs'><i class='fa fa-edit'></i></button></a>";
            ...
            $row[] = $data_materi->skor;

            
            $row[] = floor($diff/60);
            $row[] = TanggalIndo($data_materi->create_add) .'-'. date('H:i:s',strtotime($data_materi->create_add));
        ...
        $output = array(
            ...
            "recordsTotal" => $this->Hasil_latihan_M->count_all(),
            "recordsFiltered" => $this->Hasil_latihan_M->count_filtered(),
            "data" => $data,
        );
