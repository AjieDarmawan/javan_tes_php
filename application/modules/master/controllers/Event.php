...
class Event extends CI_Controller
{
    ...
    function __construct(){
        ...
        $this->load->model(array('Event_M','Materi_M','Jenis_M','Kategori_M','Paket_M'));
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $data_Event) {
            ...
            $publish = $this->db->query('select m.* from event as e inner join materi as m on e.id_event=m.id_event where m.id_event = '.$data_Event->id_event.' GROUP by m.id_event')->row();

            if($publish->publish==1){
                $name_publish = "sudah dipublish";
            }else{
                $name_publish = "BELUM dipublish";
            }
           ...
           $sess = $this->session->userdata();
            if($sess['pegawai']->role==1){
                $row[] = $edit." ".$delete." ".$materi." ".$name_publish;
            }else{
                $row[] = $edit."  ".$materi." ".$name_publish;
            }
    ...
    function simpan(){
        ...
        $id_kategori = $this->input->post('id_kategori');
         ...
         $model_event = $this->input->post('model_event');
        ...
        $data_event = array(
            ...
            'kedinasan' => $model_event,
            'id_paket'=>$id_paket,
            'no_urut_paket'=>$no_urut_paket,
